...
const Post = ({ post }) => {
	  ...
	  const { mutate: likePost, isPending: isLiking } = useMutation({
		mutationFn: async () => {
			try {
				...
				const data = await res.data.updatedLikes;
	...
	const { mutate: commentPost, isPending: isCommenting } = useMutation({
		mutationFn: async () => {
			try {
		const res = await axios.post(`/api/post/comment/${post._id}`, { text: comment });
				const data = await res.data;
                console.log("commented data",res.data);
				return data;
			} catch (error) {
				throw new Error(error);
			}
	...
	// console.log("likeby", post.likes);
	// console.log("userauth", authUser.user._id);
			...
			<div className='flex gap-2 items-start p-4 border-b border-gray-700'>
				<div className='avatar'>
					<Link to={`/profile/${postOwner?.username}`} className='w-8 rounded-full overflow-hidden'>
						<img src={postOwner.profileImg || "/noDp.png"} />
					</Link>
				</div>
